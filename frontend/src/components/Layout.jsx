...
import { Circle, ClipboardList, Clock, TrendingUp } from "lucide-react";
...
const Layout = ({ onLogout, user }) => {
  ...
  const stats = useMemo(() => {
    const completedTasks = tasks.filter(
      (t) =>
        t.completed === true ||
        t.completed === 1 ||
        (typeof t.completed === "string" && t.completed.toUpperCase() === "YES")
    ).length;
  ...
  const StatCard = ({ title, value, icon }) => (
    <div className="p-2 sm:p-3 rounded-xl bg-white shadow-sm border border-purple-180 hover:shadow-md transition-all duration-300 hover:border-purple-100 group">
            ...
            <div className="bg-white rounded-xl p-4 sm:p-5 shadow-sm border-purple-100">
              ...
              <div className="grid grid-cols-2 gap-3 sm:gap-4 mb-4 sm:mb-6">
                ...
                <StatCard
                  ...
                  icon={<ClipboardList className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-purple-500" />}
                />
              </div>
